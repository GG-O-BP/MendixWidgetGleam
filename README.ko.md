...
- **glendix 패키지** — React + Mendix API + JS Interop의 타입 안전한 Gleam 바인딩. `EditableValue`, `ActionValue`, `ListValue` 등 모든 Mendix Pluggable Widget API를 지원
...
gleam.toml                            # glendix >= 2.0.16 의존성 포함
```
...
- **[glendix](https://hexdocs.pm/glendix/)** — React + Mendix API + JS Interop Gleam 바인딩 (Hex 패키지)
