...
- **범용 React FFI 계층** — 단일 FFI 파일(`react_ffi.mjs`)에 모든 React 원시 함수를 통합하고, Gleam 모듈(`react.gleam`, `react/*.gleam`)에서 타입 안전한 API 제공
...
src/
  widget/                             # Gleam 소스 코드
    ...
    react_ffi.mjs                     #   React FFI 어댑터 (모든 React 원시 함수)
...
```
Gleam 소스 (.gleam) + FFI 어댑터 (react_ffi.mjs)
...
Gleam 함수 `fn(JsProps) -> ReactElement`는 React 함수형 컴포넌트와 동일한 시그니처다. `react_ffi.mjs`는 React 원시 함수를 노출하는 얇은 래퍼이고, Gleam 모듈 계층(`react.gleam`, `react/prop.gleam`, `react/html.gleam` 등)이 타입 안전한 API를 제공한다.
...
- **Gleam FFI** — React API 바인딩 (`@external` + `react_ffi.mjs`)
